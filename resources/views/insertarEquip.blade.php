...
<div class="estilos" style="text-align:center; margin-top:20px; margin-bottom:10px;">
    <tr>
        <td>
            <form method="POST" action="{{ route('insertadoEquip')}}">
            @csrf
                <label>
                
                    Nom de l'equip: <input type="text" name="nombre">
                    {!!$errors->first('nombre', '<small>El nombre es necesario</small>')!!}</br></br>
                    Ciudat de l'equip: <input type="text" name="ciudad">
                    {!!$errors->first('ciudad', '<small>La ciudad es requerida</small>')!!}</br></br>
